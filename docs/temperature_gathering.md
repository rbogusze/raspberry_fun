...
Looks like the Raspberry Pi's1 do not have a long career ahead, even in project like that. With one RPi2 as a consumer (MySql or Kafka so far) I am able to consume messages generated by 2 x RPi2 + 2 x RPi1. That is impressive on its own, and with Kafka it looks I can simply do more.

Replacing Raspberry Pi1 with Pi3, the fastest so far in the test.
